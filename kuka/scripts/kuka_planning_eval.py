...
parser.add_argument("--plan_num", type=int, default=1, help="how many times to interface with env")
...
def eval_episode(model, env, dataset, savepath, idx=0):
    state = env.reset()
    states = [state]
...
savepath = f'plans_unco_{H}_{T}/{seed}'
...
for i in range(plan_num):
    ...
    reward, total_samples, frames = eval_episode(model, env, dataset, savepath, idx=i)
